...
public class LoginActivity extends AppCompatActivity {
    ...
    private String name, role, gender, dob, branch, phone, email, sem,imageUrl,designation,qualifications;
    ...
    public void onClickSignIn(View view) {
        ...
        Query checkUserForFaculty = FirebaseDatabase.getInstance().getReference("root").child("profiles").child(branchSpinner.getSelectedItem().toString()).child("faculty").orderByChild("regNo").equalTo(regNo);
        Query checkUserForStudent = FirebaseDatabase.getInstance().getReference("root").child("profiles").child(branchSpinner.getSelectedItem().toString()).child("student").orderByChild("regNo").equalTo(regNo);
        checkUserForStudent.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    checkUserForFaculty.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            if (snapshot.exists()) {
                                edRegNo.setError(null);
                                ...
                                if (systemPassword.equals(password)) {
                                    ...
                                    branch = snapshot.child(regNo).child("branch").getValue(String.class);
                                    imageUrl = snapshot.child(regNo).child("imageUrl").getValue(String.class);
                                    designation = snapshot.child(regNo).child("designation").getValue(String.class);
                                    qualifications = snapshot.child(regNo).child("qualifications").getValue(String.class);
